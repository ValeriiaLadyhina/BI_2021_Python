def fastq_filter(gc_bounds, length_bounds, quality_threshold,
    ...
    parameters = input('Do you want to change any parameters? Please type Y/N.\n By default:\n\n'
                       ...
                       '* save_filtered = False\n')
    if parameters == 'Y':
        gc_bounds, length_bounds, quality_threshold, save_filtered = change_of_parameters(gc_bounds=gc_bounds, length_bounds=length_bounds,
...
def quality_control(sequence_dictionary):
    ...
    for key in sequence_dictionary:
        ...
        sequence_dictionary[key].append(round(GC_composition,1)) #GC composition index 4
        length_of_reads.append(len(sequence))
        sequence_dictionary[key].append(len(sequence)) #length index 5
        quality = []
        for i in sequence_dictionary[key][3]:
            quality.append(ord(i)-33)
        sequence_dictionary[key].append(np.mean(quality)) # quality index 6
        ...
        for i in parameters_to_change:
            ...
            if i == 1:
                gc_bounds = [int(x) for x in input('Example 0 100 - print two numbers with delimiter - space\n\n'
                                'Enter lower and upper bound for GC content in between 0 to 100.\n ').split()]
                ...
                save_filtered = input('Do you want to save both reads that passed filter and reads that did not in two '
                                      'separate files? Then please print True, if you want to save only fastq file with '
...
if __name__ == "__main__":
    ...
    and quality to filter your raw Illumina sequencing data. It is a first version of the tool, therefore I would ask
    you to be careful with your input of the name of file to be analyzed.\n
    ...
    input_fastq = input('Enter name of input fastq file including filename extension.You can enter either just name of '
                    'the file if it is located in the same folder as the script or print the absolute path to it.\n')
